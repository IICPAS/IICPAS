...
// Check all footer data
const checkAllFooterData = async () => {
  try {
    console.log("🔍 Checking all footer data...");
    ...
    footers.forEach((footer, index) => {
      ...
      console.log(
        `   Footer Links Structure: ${Object.keys(
          footer.footerLinks || {}
        ).join(", ")}`
      );
      ...
      if (footer.footerLinks) {
        ...
        if (footer.footerLinks.courses) {
          console.log(`   Courses: ${footer.footerLinks.courses.length} items`);
        }
      }
    });
