  ...
  try {
    ...
    if (!userId || !route || !duration || !ip || !sessionId) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: userId, route, duration, ip, sessionId'
      });
    }
...
export const getUserActivitySummary = async (req, res) => {
  try {
    const { userId, days = 7 } = req.params;
    
    const summary = await AuditActivity.getUserActivitySummary(userId, parseInt(days));
...
export const getIPActivitySummary = async (req, res) => {
  try {
    const { ip, days = 7 } = req.params;
    
    const summary = await AuditActivity.getIPActivitySummary(ip, parseInt(days));
